...
def dataset_filter(dataset, start_stop, destination_stop):
...
def calculate_distance(dataset, start_stop, destination_stop, hour, day, month):
    filter = dataset_filter(dataset, start_stop, destination_stop)
...
print(f"Calculating time between {first_location} and {second_location}")
calculate_distance(original_file, first_location, second_location, now.hour, now.weekday(), now.month)